================================================================================
 ARCHIVED -- the original hand-written commands, superseded by ./dl.sh
================================================================================
Kept as a reference for the raw yt-dlp invocation: if dl.sh ever breaks, or you
want to run a one-off playlist that is not in the table, this is the shape of
the underlying command.

Do NOT copy it verbatim -- it has known problems, all documented in the
"WHAT CHANGED FROM THE OLD COMMANDS" section of readme.txt:
  - transcodes opus -> mp3 (lossy to lossy) instead of copying the m4a stream
  - --parse-metadata writes literal "NA" artist/album tags on non-music videos
  - "&& find ... > .m3u" leaves the playlist file stale after a failed run
  - --download-archive missing on all but the first playlist
  - no error cap, so a broken run grinds through the whole playlist
  - --cookies-from-browser, which causes the "cookies are no longer valid" churn

Frozen from git HEAD on 2026-08-24 (last edited 2026-08-22).
================================================================================
